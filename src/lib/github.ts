...
function getOctokit(): Octokit {
  // Always create fresh instance to ensure env vars are read
  return new Octokit({
...
export async function getRepoFileTree(repoString: string): Promise<FileTreeItem[]> {
...
export async function getFileContent(repoString: string, filePath: string): Promise<string | null> {
  const { owner, repo } = parseRepoString(repoString);
...
export async function getAnalyzableFiles(repoString: string): Promise<FileTreeItem[]> {
  const allFiles = await getRepoFileTree(repoString);
  ...
  for (let i = 0; i < filePaths.length; i += batchSize) {
    ...
    const results = await Promise.all(
      batch.map(async (path) => {
        const content = await getFileContent(repoString, path);
