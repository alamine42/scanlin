...
export function GitHubConnectButton({ workspaceSlug, isConnected, username }: GitHubConnectButtonProps) {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  const handleConnect = () => {
    window.location.href = `/api/oauth/github/authorize?workspace=${workspaceSlug}`;
  };
  ...
  const handleDisconnect = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/oauth/github/disconnect?workspace=${workspaceSlug}`, {
        method: 'POST',
      });
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-300">
            Connected as <span className="text-green-400 font-medium">@{username}</span>
          </span>
        </div>
        <button
